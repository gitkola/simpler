...
export const AI_INSTRUCTIONS_PROJECT_STATE = `#Project status
...
"Project State" updates from your response will be checked and synchronized by the user with the current project state and files.
...
3. Add or edit project "files" according to the "description", "requirements" and project objective in the user request. Make sure each file has a valid relative path and appropriate content. The contents of the file must be empty until it is processed by the corresponding task. For each file in the "Project State", fill in the "status" field in accordance with the "IProjectFile" interface.
...
7. Update the task statuses in the "status" field of the "Project State" tasks with the appropriate value according to the "IProjectTask" interface.
8. Analyze the current state of the project and provide recommendations for the next steps in development by setting the "suggested_as_next_task" field of the corresponding task in the "Project State" object to true/false. Provide 3-5 specific, achievable tasks that align with the current "Project State".
9. If you haven't updated the "Project State", don't add it to your answer.
10. If you updated the "Project State", add it to the response as a JSON object with a clear indication of what was changed. For "files" update corresponding "status" field according to the "IProjectFile" interface to indicate which file was added, edited or deleted.
