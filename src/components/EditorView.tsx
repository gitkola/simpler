...
import { ToggleEditorVisibility } from "@/App";
...
export default function EditorView() {
  ...
  const {
    ...
    currentProjectOpenedFilesError,
  ...
  return (
    <div className="flex flex-1 flex-col border-r border-0.5 min-w-[900px]">
      <div className="flex items-center justify-between border-b border-0.5">
        <div className="flex items-center px-2 space-x-2">
          <button
            ...
            onClick={() => dispatch(setShowDiff(false))}
          >
          ...
          <button
            ...
            onClick={() => dispatch(setShowDiff(true))}
          >
        ...
        <ToggleEditorVisibility>
          <SquareButton
            ...
            className=""
            onClick={() => {
              // dispatch(setShowCodeEditor(false));
            }}
          />
        </ToggleEditorVisibility>
      </div>
      ...
      {currentProjectOpenedFilesError && (
        <div className="flex px-2 py-1 mx-auto mt-16 text-red-700 bg-red-300 border border-red-700 rounded-md">
          {currentProjectOpenedFilesError}
        </div>
      )}
      {!isLoadingCurrentProjectOpenedFiles && (
        <TabsView>
          {currentProjectOpenedFiles.map((file) => (
            ...
            >
              {!!file.isActive && (
                <FileViewer showDiff={showDiff} path={file.path} />
              )}
            </TabContent>
          ))}
        </TabsView>
