...
const Message: React.FC<MessageProps> = ({ message }) => {
  ...
  //           return (
  //             <div className="space-y-2" key={item.text}>{parsedText.map((item) => renderContent(item))}</div>
  //           );
  //         } else {
  //           return <p style={{ whiteSpace: 'pre-wrap' }} key={item.text}>{item.text}</p>;
  //         }
  ...
  //             className="bg-blue-500 hover:bg-blue-700 text-white font-bold px-3 rounded-full"
  //           >
  //             Write to file
  //           </button>
  ...
  // return (
  //   <div key={message.id} className={`flex flex-col items-center max-w-max select-text justify-end`}>
  //     <div className={`flex flex-col p-2 space-y-2 rounded-md bg-blue-600 bg-opacity-20 hover:shadow-md`}>
